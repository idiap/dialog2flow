...
class DenoisingAutoEncoderLoss(BaseLoss):
    ...
    def __init__(
        ...
        encoder_name_or_path = encoder_name_or_path or model[0].auto_model.config._name_or_path
        ...
        self.need_retokenization = not (type(self.tokenizer_encoder) == type(self.tokenizer_decoder))  # noqa: E721
        ...
        try:
            ...
            raise e
        assert self.encoder[0].auto_model.config.hidden_size == decoder_config.hidden_size, 'Hidden sizes do not match!'
    ...
    def save_state(self, path: str):
        torch.save({"model_state_dict": self.decoder.state_dict()}, path)

    def load_state(self, path: str):
        self.decoder.load_state_dict(torch.load(path)["model_state_dict"])
